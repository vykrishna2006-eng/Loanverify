...
function ExceptionDetail({ id, onClose, onContinue }) {
  ...
  const submitDecision = async () => {
    ...
    if (decision === 'EDITED' && !corrected.trim()) {
    ...
    try {
      ...
      if (res.data.remaining_count > 0) {
        ...
        toast(`⚠️ ${res.data.remaining_count} more issue(s) need review before verification`,
          { duration: 5000 });
        return;
      }
  ...
  const forceVerify = async () => {
    setForceLoad(true);
    try {
      const res = await exceptionsAPI.forceVerify(
        id,
        note || 'Manually verified after reviewing all exceptions'
      );
      toast.success(
        `✅ Loan ${res.data.loan_id} force-verified! ` +
        `${res.data.dismissed_count} exception(s) auto-dismissed.`
      );
      setRemaining([]);
      onClose?.();
    } catch (e) {
            ...
            <>
              ...
              <div className="card" style={{ margin: 0, padding: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12 }}>
                  ...
                  <div>
                    ...
                    <span style={{ color: 'var(--danger)', fontWeight: 500 }}>{e?.actual_value || '—'}</span>
                  </div>
                  <div>
                    <span style={{ color: 'var(--text-muted)' }}>Expected: </span>
                    <span style={{ color: 'var(--success)', fontWeight: 500 }}>{e?.expected_value || '—'}</span>
                  </div>
                </div>
              </div>
              ...
              {exc.loan && (
                <div style={{ fontSize: 12, background: 'var(--bg-hover)', borderRadius: 8, padding: '8px 12px' }}>
                  ...
                  {exc.loan.borrower_name || exc.loan.borrower_id || '—'}
                  <span style={{ color: 'var(--text-muted)', marginLeft: 16 }}>Balance: </span>
                  {exc.loan.current_balance != null
                    ? `$${Number(exc.loan.current_balance).toLocaleString()}`
                    : '—'}
                ...
                <div className="ai-box">
                  <div className="ai-box-header">
                    ...
                    <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--text-muted)' }}>
                      Confidence: {ai.confidence_score != null
                        ? `${parseFloat(ai.confidence_score).toFixed(1)}%`
                        : '—'}
                  ...
                  {ai.prompt_text && (
                    <details style={{ marginTop: 8 }}>
                      ...
                      <pre style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 6, whiteSpace: 'pre-wrap' }}>
                        {ai.prompt_text}
                      </pre>
                    </details>
                  )}
                  ...
                  {ai && (
                    <div className="form-group">
                      ...
                      <div style={{ display: 'flex', gap: 10 }}>
                        <button
                          className={`btn btn-sm ${aiFollowed === true ? 'btn-success' : 'btn-secondary'}`}
                          onClick={() => setAiFollowed(true)}
                        >Yes</button>
                        <button
                          className={`btn btn-sm ${aiFollowed === false ? 'btn-danger' : 'btn-secondary'}`}
                          onClick={() => setAiFollowed(false)}
                        >No</button>
                      </div>
                    </div>
                  )}
              ...
              {remaining.length > 0 && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    {remaining.length} more issue{remaining.length !== 1 ? 's' : ''} must be
                    clarified before this loan is verified
                  </div>
                  ...
                  {history.map(d => (
                    <div key={d.id} className="timeline-item">
                      <div className="timeline-dot" style={{
                        background:
                          d.decision === 'APPROVED' || d.decision === 'EDITED'
                            ? 'var(--success)'
                            : d.decision === 'REJECTED'
                            ? 'var(--danger)'
                      ...
                      <div className="timeline-event">
                        ...
                        {d.ai_decision_followed === true && (
                          <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--accent)' }}>
                            followed AI
                          </span>
                        )}
                        {d.ai_decision_followed === false && (
                      ...
                      {d.corrected_value && (
                        <div className="timeline-detail">
                          Field corrected →{' '}
                          <strong style={{ color: 'var(--success)' }}>{d.corrected_value}</strong>
                          <span style={{ color: 'var(--text-muted)', marginLeft: 8 }}>
                            (was: {d.original_value})
                          </span>
                        </div>
                      )}
                      {d.reviewer_note && (
...
export default function Exceptions() {
  ...
  const [excTypes, setExcTypes] = useState([]);

  const PAGE_SIZE = 20;

  useEffect(() => {
    exceptionsAPI.listTypes()
      .then(r => setExcTypes(r.data || []))
      .catch(() => {});
  }, []);
  ...
  // When reviewer clicks "Review Next Issue"
      ...
      <div className="card">
        ...
        <div className="filter-bar">
          <div className="search-input-wrap" style={{ flex: 2, minWidth: 200 }}>
            ...
            <input
              className="form-input"
              placeholder="Search loan ID, borrower, type…"
              value={search}
